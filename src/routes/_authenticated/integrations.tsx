...
import { MessageCircle, Facebook, Copy, CheckCircle2, RefreshCw, AlertCircle, Save } from "lucide-react";
import { toast } from "sonner";
import { format } from "date-fns";
import { useAppSettings, saveAppSetting } from "@/hooks/use-app-settings";
...
function IntegrationsPage() {
  ...
  const [dtUrl, setDtUrl] = useState(appSettings.doubletick_chat_url ?? "");

  useEffect(() => {
    setDtUrl(appSettings.doubletick_chat_url ?? "");
  }, [appSettings.doubletick_chat_url]);
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 max-w-6xl mx-auto animate-in fade-in duration-500">
      ...
      {/* DoubleTick App URL setting */}
      <Card className="mt-6 p-6 shadow-card">
        <div className="flex items-center gap-3 mb-4">
          <div className="size-9 rounded-lg bg-gradient-to-br from-emerald-500 to-green-600 text-white flex items-center justify-center">
            <MessageCircle className="size-4" />
          </div>
          <div>
            <h2 className="font-display font-semibold">DoubleTick Chat URL</h2>
            <p className="text-xs text-muted-foreground">WhatsApp button mein is link se DoubleTick app directly open hoga</p>
          </div>
        </div>
        <div className="space-y-2">
          <Label className="text-xs">
            ...
            <span className="text-muted-foreground font-normal">
              — <code className="bg-muted px-1 rounded">{"{phone}"}</code> ki jagah number aa jayega
            </span>
          </Label>
          <div className="flex gap-2">
            <Input
              value={dtUrl}
              onChange={(e) => setDtUrl(e.target.value)}
              placeholder="https://web.doubletick.io/conversations/91{phone}"
            ...
            <Button
              ...
              onClick={async () => {
                await saveAppSetting("doubletick_chat_url", dtUrl.trim());
                toast.success("DoubleTick URL saved");
              }}
            >
          ...
          <p className="text-[11px] text-muted-foreground">
            <code className="bg-muted px-1 rounded">{"91{phone}"}</code> → 91 + 10-digit (India code) &nbsp;·&nbsp; <code className="bg-muted px-1 rounded">{"{phone}"}</code> → sirf 10-digit
          </p>
          <p className="text-[11px] text-muted-foreground">
            Recommended: <code className="bg-muted px-1 rounded select-all">{"https://web.doubletick.io/conversations/91{phone}"}</code>
          </p>
        </div>
      </Card>
